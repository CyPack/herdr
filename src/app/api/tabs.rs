...
impl App {
    ...
    pub(super) fn handle_tab_close(&mut self, id: String, target: TabTarget) -> String {
        ...
        if closes_workspace {
            ...
            // cascade removes the state the records freeze. Upstream turned
            // the last-tab refusal into a workspace close, so the ghosts are
            // born on this road now as well — after the confirmation gate,
            // so a refused close still haunts nobody.
        ...
        // TP-AGPANEL-25: same rule on the ordinary road — records freeze
        // before the cascade removes the state they read.
        for pane_id in &pane_ids {
            self.state.note_agent_closed(ws_idx, *pane_id);
        }
