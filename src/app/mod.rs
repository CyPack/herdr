    ...
    pub fn new(
        ...
        let (
            ...
            let restored_sidebar_collapsed = restored_left_panel
                .map_or(config.ui.sidebar_start_collapsed, |preference| {
        ...
        let mut state = AppState {
            ...
            space_nodes: crate::spaces::validate_node_forest(
                config.spaces.nodes(),
            ...
            mouse_wheel_host_scroll: config.ui.mouse_wheel_host_scroll,
            ...
            per_display_focus: config.session.per_display_focus,
                ...
                crate::raw_input::RawInputEvent::Key(key) => {
                    ...
                    match key.kind {
                        crossterm::event::KeyEventKind::Press => {
                            ...
                            // The frozen shell router decides who owns this key
                            // (popup, overlay, capture, focused component, or the
                            // terminal) — TP-SBS-FOCUS: asking only whether a
                            // terminal context exists would route around the
                            // beside half's keyboard.
                            let target = self.handle_key_headless_from(source_id, key.clone());
...
mod tests {
    ...
    #[tokio::test]
    async fn route_client_input_visible_terminal_receives_bracketed_paste() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("test");
        let focused = workspace.focused_pane_id().unwrap();
        let (runtime, mut rx) = TerminalRuntime::test_with_channel(80, 24);
        ...
        app.route_client_input(b"\x1b[200~visible-terminal-paste\x1b[201~".to_vec());
