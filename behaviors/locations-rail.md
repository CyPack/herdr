...
| TP-FLF-COMPACT-VIS | The compact drawer paints the same cursor semantics as the wide Rail. | Cursor semantics change with terminal width. | `flf_compact_drawer_focus_matches_wide_rail` |
...
| TP-FLF-RENDER-01 | Focus changes alter paint only: never state, row hit targets, column projection, or repeated output bytes. | Moving focus mutates hit geometry, so clicks resolve to different rows than the ones drawn. | `flf_render_is_state_pure_and_geometry_identical` |
...
| TP-FLF-STEP-RENDER | Routine `compute_view` reconciliation cannot snap a still-valid keyboard cursor back to the accepted origin. | The cursor silently jumps home on an unrelated repaint. | `flf_reconcile_preserves_valid_cursor_distinct_from_origin` |
| TP-FLF-VIS-01 | The keyboard target is the strongest Rail identity and the accepted origin stays visible without impersonating current focus; ANSI and no-color users can still tell cursor from origin through modifiers when every relevant colour agrees. | On restricted-colour terminals the cursor and the origin become indistinguishable. | `flf_render_no_color_distinguishes_cursor_from_origin_by_modifiers`, `flf_render_rail_cursor_wins_and_origin_remains_subdued` |
...
| TP-FLF-BOUNDED-TP | The same calibration pairing for the blocked-lane observation. | As above. | `flf_scale_locations_follow_navigation` |
...
| TP-FLF-FAIL-STALE | Failure markers carry cursor, generation and revision authority rather than path alone. | A stale error is shown against the current destination. | `flf_render_pending_failure_apply_only_to_current_cursor` |
...
| TP-FLF-STALE-01 | Replacing the model retires request and error authority tied to an obsolete cursor before selecting the new first accessible row. | An error from the previous model is attributed to the new one. | `flf_cursor_reconcile_retires_obsolete_pending_and_failure` |
...
- `TP-FLF-BOUNDED-01` and `TP-FLF-BOUNDED-TP` are deliberately excluded from
