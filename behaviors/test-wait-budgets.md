...
| TP-PLUGIN-CAP-01 | A plugin capture is read only once it has stopped growing, not once it is merely non-empty | A plugin writes its capture a line at a time, so the first byte can land several lines before the last. A reader that returns on the first byte gets whatever happened to be flushed, and the test then fails about a missing line rather than about the wait — a complaint pointing at the content instead of at the waiting. `2fe57a57` stabilised these tests with the non-empty rule and the flake got rarer rather than gone; it surfaced again under a loaded full suite. The row is deterministic: the capture is grown from inside `pump`, so it fails against the non-empty rule every time | `a_capture_still_being_written_is_not_read_until_it_settles` |
