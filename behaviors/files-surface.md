...
| TP-FIP-FORMAT-06 | With the graphics protocol switched off, a picture or PDF panel names the exact config key (`experimental.kitty_graphics`), sized to the panel so the key survives truncation. | Naming the capability instead sends the reader hunting for a missing terminal feature; the terminal is usually fine and the flag is a one-line edit they cannot guess. | `image_preview_has_explicit_non_kitty_fallback_and_ready_content_is_clear` |
...
| TP-FIP-REF-20 | The reference delivery sweep enters the RAISING display's view before checking that the chosen path is still on that browser's screen; the request carries the raising client id from the picker. | The sweep runs outside any viewer window, so the check read whatever display was parked in the registers — the moment a second client attached, every send died as "authority changed" while the raising display's browser plainly still showed the file. | `delivery_validates_against_the_raising_displays_browser`, `activation_stamps_the_raising_display_on_the_request` |
...
| TP-FMW-REFRESH-05 | A refresh re-projects the column from the trail CURSOR authority (`cursor_path_in_col`: a vertical override wins, the activated selection is its fallback), restores an override that re-marking the selection cleared, and prepares the detail panel for that focused row. | The two-second reconcile drags the highlight and the detail panel back to the activated row, so a couple of seconds after every click the focus jumps to the top of the column. | `a_reconcile_refresh_leaves_the_clicked_row_focused` |
...
| TP-FPOPUP-02 | A plugin pane opened with a title wears that title, not the one its manifest declares. | The popup is the one placement nothing can rename afterwards — `parse_pane_id` resolves ids through a workspace's pane tree and a popup lives outside every tree — so a name it does not receive at open it never receives at all. The preview click would keep labelling a picture with the name of the plugin that opened it. | `a_popup_pane_opens_under_the_title_its_caller_asked_for` |
| TP-FPOPUP-03 | A caller that passes no title, or one that is only whitespace, leaves the manifest's title in place. | Every plugin installed today omits the field, so an unconditional override would strip the name off every plugin pane on upgrade; and a shell filling the flag from a file name can hand over an empty string, leaving a pane with no name and nothing to explain why. | `a_popup_pane_without_a_usable_title_keeps_the_manifest_name` |
| TP-FPOPUP-04 | The caller's title is honoured at every placement, not only the popup that motivated it. | A parameter four placements accept and three quietly ignore is a worse trap than the wrong title it replaced: the plugin author sends it, nothing happens, and nothing says why. | `a_plugin_pane_opened_in_a_tab_also_takes_its_caller_s_title` |
