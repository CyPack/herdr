...
| TP-DRAW-15 | The age column and the ordering read one clock, preferring the transcript's own last user/assistant message `timestamp`, then the file mtime, then the ledger sighting. | A server restart re-sights every open chat and a resume touches every transcript's mtime — with either as the reference every age in the panel resets to "now" on restart, which is exactly the defect the user reported. | `the_age_clock_prefers_the_last_message_over_mtime_and_sighting`, `the_last_message_timestamp_comes_from_the_last_user_or_assistant_line`, `a_malformed_timestamp_does_not_poison_the_last_good_one`, `the_listing_orders_by_the_message_clock_not_the_file_clock`, `a_restart_cannot_reset_the_drawer_age_to_now`, `a_daily_or_module_row_age_survives_a_restart_too`, `the_phone_drawer_age_survives_a_restart`, `an_out_of_order_transcript_still_reports_the_newest_message`, `a_projects_tab_chat_age_survives_a_restart_too` |
| TP-DRAW-16 | Only a substantive message advances the chat clock: a user line whose text is real (non-empty, not an injected `<…>` block, not a continuation/interrupt banner) or an assistant line carrying visible text — and never a timestamp in the future beyond a small skew allowance. This mirrors the content sieve of the trusted external lister (`claude-sessions` / `cc l`). | While an agent runs, every tool call appends timestamped user/assistant machinery lines; counting them pins the row at "now" forever — the user's "always 1m ago" chat. One corrupt future-dated line has the same effect through `max`. | `tool_use_only_assistant_lines_do_not_advance_the_clock`, `an_assistant_line_with_text_advances_the_clock`, `injected_angle_bracket_blocks_do_not_advance_the_clock`, `continuation_and_interrupt_banners_do_not_advance_the_clock`, `tool_result_user_lines_do_not_advance_the_clock`, `a_future_dated_line_is_ignored_by_the_clock`, `the_clock_matches_the_cc_l_content_sieve_on_a_mixed_transcript` |
...
| TP-DRAW-14 | The chat history is a named step both constructors call: a handed-off server reads the ledger and the transcript store even though it passes `no_session`. | TP-DRAW-13 put the transcript read behind `no_session`, which is right for what that flag means on a normal start and on a fixture — leave the machine's history alone. It is wrong for a handoff, which passes the same flag to mean something narrower: restore the session from a snapshot rather than from disk. One word answering two questions cost the drawer its history on the only path that matters here, because on this machine every delivery is a handoff. The seeding of the graveyard sits behind the same step for the same reason, and both are named rather than inlined so a third constructor cannot inherit the silence. | `a_handed_off_server_loads_the_chat_history` |
...
frame.
| TP-CHATROW-ID-01 | Every laid-out chat row — workspace drawer, daily section, container — carries the chat's `session_id` instead of a list position, resolved at compute time while the rect and the ledger still agree. A press opens exactly the chat the user saw (`open_*_chat` take the id; a chat that left the ledger is a no-op), and the renderer paints each row by the same identity, so a ledger that gains a row between two frames can neither open nor title the wrong chat. The mobile drawer bridges by resolving the id at the press site until its targets carry identity themselves. | The old rects carried `chat_idx`; the ledger is rewritten newest-first while frames are in flight, and an index resolved against the new list opened whichever chat had shifted into the slot — the "wrong chat in the wrong directory" family (#46's sibling), triggered by nothing more than another client filing a chat. | `a_stale_daily_row_press_opens_the_chat_the_user_saw`, `a_click_on_a_daily_row_that_no_longer_exists_does_nothing`, `a_daily_chat_resumes_in_the_daily_directory`, `a_filed_chat_in_a_bucket_resumes_in_its_repository` |
| TP-CHATROW-ID-02 | The laid-out chat-row areas resolve their `session_id` from the SAME ledger every other drawer surface reads — `workspace_chat_rows_for`, keyed by the checkout (TP-WSID-03) — never from the birthplace `identity_cwd` key. | The live incident, verbatim from the user's screen: a worktree-backed workspace badged "14" chats drew ONE and left a region of blank rows — compute measured rows out of one ledger and resolved identities out of another, so the rows existed, consumed height, and could never be found to draw. | `chat_row_areas_read_the_ledger_the_drawer_reads` |

## Work log sheet

| ID | Behavior | Breaks if lost | Verified by |
|---|---|---|---|
| TP-WORKLOG-01 | "Work log..." on a chat row opens a modal sheet of that chat's CONFIRMED commits, grouped feature-first/fix-second in a fixed kind order, chronological inside each group, with scope · branch · short-sha and a pushed marker — wired into BOTH context-menu bodies, drawn and scrolled on the keybind sheet's exact contract. | The user named the defect this answers: one chat touches ten areas and its commits drown in the transcript, legible only as numbers. Wire the verb into one menu body and it is #91 again — green tests, dead affordance. Invent a second modal language and the app stops feeling like one program. | `both_context_menu_bodies_open_the_worklog_sheet`, `the_sheet_groups_commits_by_kind_with_features_first`, `an_unconfirmed_attempt_is_not_listed` |
| TP-WORKLOG-02 | The verb appears only on a chat the analyzer recorded confirmed commits for, resolved at menu-open like its sibling conditions. | An empty sheet on every conversational chat is noise that teaches the reader to stop opening it; a pick-time lookup can diverge from what the menu promised when the log reloads underneath it. | `the_worklog_verb_shows_only_for_a_chat_with_confirmed_commits` |
| TP-WORKLOG-03 | The log file is the analyzer's, read-only to the runtime: missing, stale or corrupt degrades to "no work log", never to a startup failure, and an attempt without a sha is not listed as work done. | A hand-edited or half-written file must not stop the server (the TP-WSCHAT-08 posture on a second file), and counting failed attempts as commits pollutes the chronology the sheet exists to clarify. | `a_missing_or_corrupt_worklog_degrades_to_empty_without_panicking`, `the_analyzers_file_shape_round_trips` |
