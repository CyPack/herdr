...
| TP-WSCHAT-19 | The drawer toggle is the workspace row's right-edge cell, offered only where there is history; it opens AND closes, and the rest of the row still selects the workspace | The left edge already carries the worktree-group chevron, so sharing a cell makes one toggle unreachable; an arrow on every row that only ever reveals "(no chats)" is noise; a toggle that swallows the row makes the workspace unclickable | `only_a_workspace_with_history_offers_a_drawer_toggle` · `the_drawer_toggle_opens_and_closes_without_stealing_the_workspace_click` |
