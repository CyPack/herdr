...
| TP-FOCUS-03 | Arriving at a workspace opens its chat drawer; a fold made while standing in it holds until the user leaves and returns, and an empty history is never revealed onto its placeholder. | The chat the accent descends to hides behind a fold the reader has to know to open — the "hidden drawer" the user reported. | `activating_a_workspace_reveals_its_chat_drawer`, `a_fold_made_inside_the_workspace_holds_until_reactivation`, `an_empty_history_is_not_revealed_on_activation` |
