...
| TP-DOTS-01 | The node header's menu offers creation (new sub-module under the header, new parallel module beside it) plus the one fold verb the current state calls for; the bucket header's menu only folds, because a split rule cannot parent a node. | Tree management stays discoverable only from branch rows, or the bucket menu promises a sub-module the tree cannot hang. | `header_menus_offer_creation_and_the_right_fold_verb` |
...
| TP-DOTS-03 | "⋯" is mouse chrome on every tree level — the workspace card (one column left of the "+") and both header rows wear it while the mouse owns the sidebar, and none of them otherwise. | The management menus stay invisible until someone happens to right-click, which is the discoverability complaint that produced this family. | `the_manage_dots_are_mouse_chrome_on_every_tree_level` |
