...
| TP-MOB-70 | Only the active workspace spends a second row on its detail line. | The detail answers "what state is this branch in", asked about one branch rather than sixteen. Measured: sixteen workspaces made a 42-row document in a 32-row viewport, putting the menu — and `select text` with it — five rows below the fold. | `only_the_active_workspace_spends_a_second_row_on_detail`, `a_tight_drawer_gives_each_entry_a_single_row` |
| TP-MOB-71 | Sixteen workspaces fit the drawer without scrolling. | `select text` is the way back out of a mode where taps reach nothing; a way out that needs five rows of scrolling first is not one. | `the_drawer_fits_the_reader_s_real_workspace_count` |
