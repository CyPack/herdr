...
| TP-SPLIT-CONF-02 | A duplicate key inside one repository is reported, because two rules of one repo sharing a key silently merge two of its modules into one space. | The merge reads as a grouping bug and is debugged in the sidebar instead of in the config. | `spaces_diagnostics_report_duplicate_keys` |
...
| TP-SPLIT-CONF-04 | The same key across different repositories is deliberate: it gathers both repositories' checkouts under one module group, with no warning banner. | A cross-repo module (fork branches + a sibling repo under one "Web" module) either spams a persistent config warning or silently stops merging. | `spaces_diagnostics_ignore_shared_key_across_repos`, `two_repositories_sharing_a_space_key_render_one_module_group` |
