...
| TP-SPEC-12 | The bar keys the spec lists are exactly the ones the config reference documents, in both directions | The direction TP-SPEC-06 cannot reach. Feeding every listed key through the parser proves each one is real, but says nothing about a key that is real and was never listed — and a spec that quietly omits a key is a feature nobody can find, which is what this whole surface exists to remove. The reference is a separate artefact with a gate of its own, so the two lists are drawn from different places; two lists drawn from one place agree by construction and prove nothing | `the_bar_keys_the_spec_lists_are_the_ones_the_config_reference_documents` |
| TP-SPEC-13 | Every range a switch states is the range this build actually enforces, at both ends and one step beyond each | The ranges are hand-written text, which is exactly the surface a staleness gate exists for. Reading the bounds back out of the sentence and putting them to the checker means the spec cannot advertise a range nobody honours — the same shape as the example check, applied to the half of the grammar that is a number rather than a name | `every_range_a_switch_states_is_the_one_this_build_enforces` |
| TP-SPEC-14 | The spec publishes the colour names a bar config may write, and the same set the parser resolves | Colours were the one vocabulary in this grammar that nothing could teach. An unrecognised colour is not refused — it warns into the log and comes back cyan — so no refusal message ever names the set, and the guide showed only `"mauve"` inside examples. A reader had no way to learn that `teal` exists, which is precisely the gap `shell spec` was built to close for every other surface | `the_spec_still_carries_every_surface_it_is_meant_to`, `the_spec_names_exactly_what_the_refusals_offer`, `the_text_rendering_carries_every_name_the_json_carries` |
| TP-SPEC-15 | The spec publishes the rows a bar section's menu draws | A menu row exists only after somebody has already right-pressed, so `shell spec` is the one place a reader — an agent most of all — can learn what `secondary = "menu"` will offer before deciding whether to write it. Three, not "some": the rows are a closed set, and a spec publishing two of them would teach that a section reaches two places | `the_spec_still_carries_every_surface_it_is_meant_to`, `the_spec_serialises_to_json_a_consumer_can_read` |
