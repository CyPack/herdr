...
### Added
- A program that enabled SGR pixel mouse mode (DECSET 1016) now receives real pixel coordinates for clicks, wheel and motion that the client reported as cells: the cell is converted to its pixel centre instead of downgrading the report to cell numbers that a 1016 consumer would misread as pixels — this is what parked every click on an embedded browser's toolbar. A terminal without pixel geometry keeps the old cell-shaped report.
- A press on a pane whose program never asked for the mouse is no longer lost when that pane is painting graphics: Herdr publishes it as a `pane.pointer` event (`events.subscribe` with `{"type":"pane.pointer"}`) carrying the press kind, button, cell and — when the client reported them — pixel coordinates, so a bridge can deliver the click to whatever is drawing, such as a browser embedded over CDP. A program that enabled a mouse mode keeps receiving presses directly and never sees the event, and the subscription starts at the moment it is made, so nothing clicked before it is replayed.
- `[shell.bars.<edge>] style` names the bar's whole look in one line — `framed` (the default), `islands`, or `plain` — by filling in unwritten `border` keys; anything written explicitly still wins, in both directions. And adjacent sections naming the same `group` now share a single island frame, so cpu, mem and swap can sit in one box: the frame's two cells are charged at the group's edges, a torn group or a disagreeing colour is refused by name, and a press inside the group still lands on the section it is over.
- A bar section can be its own hide button: `action = { kind = "hide" }` switches off the edge the section sits on, the way the sidebar's collapse control folds the sidebar — except a bar is composed by you, so the button is too. `hide` reads no keys at all, and anything written beside it is refused by name rather than silently ignored. The way back is `prefix+shift+b`.
- `prefix+shift+b` switches every configured edge bar off at once, and back on with the same press — the sibling of `prefix+b` for the sidebar. Rebind it with `keys.toggle_bars`. The switch is session-local (a restart brings the bars back), it never changes what the session file stores, and it is independent of focus mode: either can quiet a bar, and a bar draws only when both let it through.
- A bar section can be drawn as its own framed island: `border` and `color` under `[[shell.bars.<edge>.sections]]` put one part of a bar in a box of its own, so cpu and memory can sit in one frame and the clock in another. The size a section asks for keeps describing what goes inside the frame — a `cells = 10` section showing a ten-cell picture still shows all ten — and the two cells the frame costs are charged on top. A frame also needs two cells across the bar plus one for what it was drawn around, measured against what `size` leaves after the bar's own border: a bar with `border = true` spends two of its own, so it needs `size = 5` before an island fits. Asking for one that does not fit leaves the bar undivided and says so, naming both numbers, rather than drawing a box with nothing in it. A section that names no colour wears its bar's rather than the global default.
- `herdr shell spec` now publishes `section_keys`, the keys a section takes whatever its `kind` is, in both the text and the JSON rendering. They belong to no kind table — every sizing kind reads them — so listing them under one would have taught that the others turn them down.
- An edge bar can go quiet in focus mode with `[shell.bars.<edge>] hide_when_focused`. Focus narrows the display to what is being worked in, and a strip that exists to be glanced at is often what that mode is asking to be rid of. Per edge, because the reason to quiet a status strip is not a reason to quiet the strip holding the controls in use, and off by default so every bar written before this key keeps behaving the way it did. The filter sits on the frame rather than on the stored value, so a session saved under focus does not come back missing a bar.
- The Files rail now offers the volumes the host has mounted as locations, read from the kernel mount table under the roots a desktop treats as places. The rail used to hold a single Root row, which is thin on a desktop and the whole rail on a server, a container or an SSH session, where there is no bookmark list and none of the well-known directories exist.
- The well-known user directories in the Files rail are read from the person's own `user-dirs.dirs`, so a localized desktop shows the names and paths it actually uses rather than the English defaults.
...
- `herdr shell spec` prints the whole edge-bar grammar, as text or as JSON with `--json`: every sizing, widget and action kind with the keys it reads and the keys it turns down, a working example for each, the metric names and their aliases, the colour names, the switches outside the bars that change what a bar may do, and the bundled pictures with the size each draws at. It needs no running Herdr, so a question about what a bar config may say can be answered before anything is started.
- An edge bar section can draw the recent shape of a metric with `widget.kind = "sparkline"`, alongside the `resource` and `meter` widgets that show its current value.
...
- Two more things a bar section can do when it is pressed. `action.kind = "run"` executes `argv` and opens nothing — detached, in the focused pane's directory, output discarded — for the things you do to a session rather than in it. `action.kind = "workspace"` switches to an open workspace by the name the sidebar shows, and says so if nothing answers to that name.
- Four more metrics for `resource`, `meter` and `sparkline` sections: `disk` (space used on the filesystem Herdr is running from), `battery` (charge remaining), `temp` (the warmest sensor the machine reports) and `net` (bytes per second in and out, loopback excluded). `net` is a figure and never a bar — a rate has no ceiling to be a proportion of. Anything the machine cannot report shows `--` rather than a number, so a desktop with no battery does not read as one that is flat.
- Only the metrics a section actually shows are read now. A bar with a `cpu` meter no longer costs a `statvfs`, a walk of the battery directory or a scan of every thermal zone every two seconds.
- An edge bar section can show the time with `widget.kind = "clock"`. `format` writes it — `%H`, `%I`, `%M`, `%S`, `%p`, `%a`, `%d`, `%b`, `%m`, `%Y`, `%y`, with everything else carried through and `%%` for a literal percent — and defaults to `%H:%M`. The format also decides how often the clock is redrawn: naming `%S` refreshes every second, leaving it out refreshes once a minute on the minute, and a bar with no clock schedules nothing at all.
...
### Fixed
- Remote sessions (SSH, tmux) now report sub-cell mouse positions again, so small targets such as form fields inside an embedded browser are clickable. Previously the pixel-mouse capability was tied to direct graphics, which relayed transports refuse, and every click snapped to the cell grid.
...
### Fixed
- Focus mode now quiets a container with nothing live under it. A container's header came from the parent-child map rather than from its members, so filtering the members left the header — and the chats moved into it, and its empty-module line — standing in a tree whose whole job is to show what you are working in. A chat open in a tab right now still keeps its container: a filter may narrow what you see, it may never hide where you are.
- Reloading the config now applies the Spaces focus filter to the running client. The reload path copied the spaces block but never the flag derived from it, so editing `focus_only` by hand and reloading did nothing: the file said one thing and the screen another, with no way to tell which was real.
...
- Workspace rows in the phone drawer grew their desktop affordances: a chat disclosure at the head of each row opens that branch's history without switching to it, a trailing `+` starts a chat there, and each remembered chat shows how long ago its last message landed.
- The phone header shows a steady dot instead of a spinner while an agent works, so the phone's terminal stops redrawing eight times a second for hours and the radio can sleep between real changes.
- The phone drawers pin their create action and `select text` to a footer band at the bottom of the panel, where scrolling cannot move them — `select text` is the way back out of a mode where taps reach nothing, and it used to sit below the fold on a full workspace list.
- Attaching a phone beside a desktop no longer mixes the two shells together: the drawer, its cursor, its chat fold and `select text` now belong to the display they were opened on, and a swipe on the phone keeps the phone's scroll behaviour while the desktop is drawing.
- The phone's spaces drawer now shows the remembered chats of the workspace you are in without anything to press first, and spends a second row on the detail line only for that workspace. Sixteen workspaces used to make a list taller than the screen, which put the menu and `select text` below the fold.
