...
Writing more than eight sections, naming a `kind` Herdr does not know, or giving a content section a `max` below its `min` leaves that bar undivided rather than guessing: one unreadable section would otherwise renumber every section after it, and the number is how a section is addressed.

### Putting something in a section

A section is empty until it is given a widget. Today there is one:
...
The label is clipped to the section by display width, so an icon that takes two cells is counted as two. A label never changes how wide its section is: the sizing rules above decide that, and text that does not fit is cut rather than pushing its neighbours around.
