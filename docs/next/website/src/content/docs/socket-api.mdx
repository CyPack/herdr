...
| Pane | `pane.split`, `pane.web.open`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.audio.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.sleep`, `pane.wait_for_output` |
| Popup | `popup.close` |
...
`pane.sleep` puts a retired, unwatched pane to sleep: its scrollback is
written to disk and its runtime is released. A pane whose child still runs
answers `child_still_running` and a watched pane answers `pane_watched` —
dormancy never touches live or watched work. Success returns the pane's info
with `dormant: true`; repeating the call on a sleeping pane is an idempotent
success. Pane info carries `dormant` and `alternate_screen` only when true,
and every dormancy transition (sleep, sweep, wake) emits a `pane.updated`
event. A dormant pane wakes when it receives input or a display starts
watching its tab.
...
`pane.web.open` splits like `pane.split` but births the new pane running a
program — `command` (default `terminal-browser open <url>`) — with
`HERDR_WEB_PANE=1` in its environment. It takes the same `workspace_id`,
`target_pane_id`, `direction`, `ratio`, `cwd`, `focus` and `env` fields plus `url`,
`command` and `agent` — the conversation the pane reports into, named by public
pane id or agent name; unwritten, the target pane's own agent is taken, else the
tab's only agent, else the pane opens unlinked. The reply's `pane_info` (and
every web pane in `pane.list`) carries a `web` object — `agent_pane_id`,
`agent_session`, `url`, `link_state` (`linked`/`stale`/`orphan`/`unlinked`) — and
the pane's environment names the tie as `HERDR_WEB_LINKED_AGENT` and
`HERDR_WEB_SESSION`. The strip's browser button, its key binding and the CLI all
ride this one method.
...
`pane.audio.stream` feeds a pane's audio to every connected client that
negotiated an Opus sink, the same way: open a dedicated socket, wait for `ok`,
then write whole 20 ms frames back to back — 48 kHz stereo interleaved
little-endian f32, exactly 7680 bytes each, with no per-frame header. A body
that is not a whole number of frames closes the stream as failed. Closing the
socket on a frame boundary ends the stream. `sample_rate_hz`, `channels` and
`format` default to `48000`, `2` and `"f32le"` and are the only values accepted.

```json
{"id":"audio_stream","method":"pane.audio.stream","params":{"pane_id":"w1:p1"}}
```
...
`pane.pointer` publishes a mouse press or release on a pane whose program
never enabled terminal mouse reporting, when that pane is painting graphics —
either an API layer (`pane.graphics.set` or a stream) or terminal-native kitty
graphics transmitted by the pane's own program, such as an embedded browser. The event carries `pane_id`, `kind`
(`down` or `up`), `button` (`0` left, `1` middle, `2` right), the cell
`column` and `row`, and `x_px`/`y_px` when the client reported pixel
coordinates. Pushed lines carry `"event": "pane_pointer"` with
`"type": "pane_pointer"` inside `data`, the same snake_case spelling every
other event kind uses on the wire. A subscription starts at the moment it is
made, so presses from before it are not replayed. A program that enabled a mouse mode keeps
receiving presses directly and no `pane.pointer` event is emitted for it.
