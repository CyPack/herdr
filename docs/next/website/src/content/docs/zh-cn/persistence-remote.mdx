...
每个抽屉的创建操作——左侧是 `+ new workspace`，右侧是 `+ new tab`——与 **select
text** 一起固定在底部的页脚区。该区域不随列表滚动，因此从 select text 返回的途径始终
只需一次点击。
...
左侧抽屉显示与桌面侧边栏相同的树，共三层：仓库、其下的检出，以及每个检出记住的会话。
点击仓库行会折叠它的链接 worktree，再次点击则展开；它们分支自的那个检出始终保留，因此
你不会迷失位置。

你正在使用的工作区会直接显示它记住的会话，并且它的行会多出一行，写明分支与代理状态。
其他工作区各占一行。点击你已经在的那一行会折叠它的会话并保持抽屉打开；点击其他行则
切换过去并关闭抽屉。

早先的版本把这棵树压平成一份检出清单。来自不同仓库的 worktree 并排排列却没有任何标识，
而记住的会话在手机上根本无法到达。
