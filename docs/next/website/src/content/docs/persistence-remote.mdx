...
Each drawer keeps its create action — `+ new workspace` on the left, `+ new
tab` on the right — pinned to a footer band at the bottom, beside **select
text**. The band never scrolls with the list, so the way back out of select
text is always one tap away.
...
The left drawer shows the same tree the desktop sidebar shows, at three levels:
the repository, the checkouts under it, and each checkout's remembered chats.
Tapping a repository row folds its linked worktrees away and tapping it again
brings them back — the checkout they branch from stays either way, so you never
lose your place.

The workspace you are in shows the chats it remembers straight away, and its row
carries a second line naming its branch and agent state. Every other workspace
takes one row. Tapping the row you are already on folds its chats away and
leaves the drawer open; tapping any other row switches to it and closes.

Earlier releases flattened this to a single list of checkouts. Worktrees from
different repositories sat side by side with nothing saying which was which, and
remembered chats could not be reached from a phone at all.

### Scrolling the terminal

Your client reports a swipe as a mouse wheel event. On a desktop that wheel goes
to the program running in the pane, which is what lets you scroll inside its own
lists and viewers. Coding agents ask for mouse reporting but usually do nothing
with the wheel, so on a phone that handover left the content area looking frozen.

In the mobile shell a vertical swipe over the terminal scrolls Herdr's own
scrollback instead. Programs that scroll with arrow keys, such as `less` and
`vim`, are unaffected — their swipe still reaches them. On a desktop-width
